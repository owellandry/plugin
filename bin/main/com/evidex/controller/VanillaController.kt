...
class VanillaController(
    ...
    override fun update(deltaTicks: Float) {
        val multiplier = if (sprinting) 3.0 else 1.0
        val step = speed * multiplier * deltaTicks.toDouble() / 20.0

        val yawRad = state.yaw.radians
        val pitchRad = state.pitch.radians

        val forwardVec = Vec3d(
            -kotlin.math.sin(yawRad).toDouble() * kotlin.math.cos(pitchRad).toDouble(),
            -kotlin.math.sin(pitchRad).toDouble(),
            kotlin.math.cos(yawRad).toDouble() * kotlin.math.cos(pitchRad).toDouble()
        ) * step
        ...
        if (forward) { dx += forwardVec.x; dy += forwardVec.y; dz += forwardVec.z }
        if (backward) { dx -= forwardVec.x; dy -= forwardVec.y; dz -= forwardVec.z }
